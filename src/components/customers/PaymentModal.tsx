...
import type { PurchaseRecord, Payment } from '../../types/customer'
...
type Props = {
  ...
  onClose: () => void
  onUpdated?: (updatedRecord: PurchaseRecord) => void
}
...
export default function PaymentModal({ record, onClose, onUpdated }: Props) {
  ...
  async function loadPayments() {
    try {
      const res = await api.payments.list(record.id)
      setPayments(res.data)
    } catch { /* empty */ }
  }

  useEffect(() => { loadPayments() }, [record.id])
  ...
  async function onAddPayment(values: PaymentFormValues) {
    setSubmitting(true)
    try {
      const updated = await addPayment(record.id, values)
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 sticky top-0 bg-white z-10 rounded-t-2xl">
          <div>
            <h2 className="font-semibold text-slate-900">การชำระเงิน</h2>
            <div className="flex items-center gap-2 mt-0.5">
              <p className="text-xs text-slate-400">ใบซื้อ {formatDate(record.date)}</p>
              <span className={`text-xs px-2 py-0.5 rounded-full ${STATUS_CLASS[record.payment_status]}`}>
                {STATUS_LABEL[record.payment_status]}
              </span>
            </div>
          </div>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-slate-50 rounded-xl px-4 py-3 text-center">
              ...
              <p className="font-semibold text-slate-900 text-sm mt-0.5">฿{record.total.toLocaleString()}</p>
            </div>
            <div className="bg-green-50 rounded-xl px-4 py-3 text-center">
              <p className="text-xs text-green-600">ชำระแล้ว</p>
              <p className="font-semibold text-green-700 text-sm mt-0.5">฿{record.paid_amount.toLocaleString()}</p>
            </div>
